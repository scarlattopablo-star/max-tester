...
5. **COLORES DEL ECO CUERO (corrección de Pablo):** las 3 fotos de costuras (ocre —NO decir "naranja"—, azul y blanca) eran del **ECO CUERO**, no del capitoneado. El capitoneado volvió a Negro/Rojo; tool nueva `mostrar_ecocuero` (fotos en `public/ecocuero/`) que Max usa SIEMPRE que preguntan colores del eco cuero. Pespuntes en config: Ocre/Azul/Blanco.
   - ⚠️ **CORREGIDO EL 17 AGO 2026 (esto de arriba era falso, commit `ceb8173`):** los 3 archivos de `public/ecocuero/` son **copias byte por byte** de los de `public/capitoneado/` → esas fotos son del **CAPITONEADO**, el eco cuero NO tiene fotos propias. Max le mandaba a una clienta "3) Eco cuero negro - Costura blanca" con precio de eco cuero ($6.500) y le vendía una cosa mostrándole otra. Ahora el pie dice **solo "Capitoneado premium - Costura ocre/azul/gris"** (nunca "eco cuero") y `economico.muestras` apunta directo al archivo del capitoneado (misma URL = no se manda dos veces). La costura clara se llama **GRIS**, no blanca (en la muestra real es gris plata), tanto acá como en los 5 colores del capitoneado. La línea desde la que se mostró la foto viaja aparte, en la nota interna del historial (`(línea: eco cuero y capitoneado premium)`), para que `eleccionAmbigua` siga frenando el cobro de la línea equivocada. Prueba: `node test_captions_capitoneado.mjs`. 👉 **Cuando Pablo pase fotos REALES del eco cuero:** van a `public/ecocuero/` y las 3 URLs de `economico.muestras` vuelven a apuntar ahí, con el pie diciendo "Eco cuero".
...
8. **Capitoneado con los 5 COLORES** (Negro, Rojo, Negro con costura ocre/azul/**gris** — era "blanca" hasta el 17 ago 2026): las costuras valen para el capitoneado Y el eco cuero. "OCRE", nunca "naranja"; y a la clara decile **GRIS**, no blanca.
