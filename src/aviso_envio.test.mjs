...
test("el aviso dice lo que pidió el dueño: 2 a 3 días", () => {
  assert.match(AVISO_ENVIO, /2 a 3 d[ií]as/i);
...
test("se borra el plazo que inventó el modelo, NO el resto del mensaje", () => {
  ...
  assert.match(salida, /2 a 3 días como máximo/, "no salió el plazo oficial");
});
...
test("venta de retiro: el mensaje sale sin ningún plazo", () => {
  ...
  assert.doesNotMatch(salida, /2 a 3 días/);
